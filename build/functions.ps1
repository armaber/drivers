...
function EvaluateNtddi
{
    ...
    if ($NtddiVersion -in 'NTDDI_WIN10_CU', 'NTDDI_WIN11_ZN', 'NTDDI_WIN11_GA') {
        ...
        $value = "0x{0:X8}" -f $script:NtddiTable.$NtddiVersion;
        $warning = @"

If you are using Windows Driver Kit v10.0.26100, then msbuild toolchain does not recognize
NTDDI_WIN10_CU, NTDDI_WIN11_ZN, NTDDI_WIN11_GA. 

Use NTDDI_WIN11_GE as alternative for the build error:
    Unknown or unsupported property value '$value' for _NT_TARGET_VERSION and Target OS Windows10

Open "${env:ProgramFiles(x86)}\Windows Kits\10\build\10.0.26100.0\WindowsDriver.OS.Props"
